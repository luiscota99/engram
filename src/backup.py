...
def run_backup(git_sync=False):
    ...
    if git_sync:
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", f"chore: automated memory backup {timestamp}"], cwd=base_dir, check=True)
            ...
            try:
                ...
                print("  (Note: No remote configured or push failed, but commit was saved locally.)")
        except subprocess.CalledProcessError:
