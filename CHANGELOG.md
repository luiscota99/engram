...
- **`read_only` safety flag on reflexes (schema v18)** — a dimension orthogonal to `kind`, learned from a prior design's per-tool `read_only` catalog. Mutating is the safe default (a script never earns free-run treatment by inference — only by explicit `engram reflex approve --read-only`). Read-only reflexes invoked by an agent over MCP run freely; **mutating reflexes now require an elicitation confirmation per use** (approval grants the capability, the confirmation authorizes this use — the two-tier trust model), closing a real gap where `handle_reflex_call` executed any approved reflex with no per-use gate. MCP tool descriptions are tagged `[read-only]` / `[mutating — asks for confirmation]` so agents know before calling.
